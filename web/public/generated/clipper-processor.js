...
class ClipperProcessor extends AudioWorkletProcessor {
  ...
  // ('sd1' = Boss SD-1 overdrive, M8; anything else = RAT, M3). Both share the
  // same handle ABI (create / set_param id 0/1/2 / set_oversampling / latency /
  ...
  _latency() {
    ...
    for (const node of this._chain) {
      if (node.engaged) n += this._pedalLatency(node);
    }

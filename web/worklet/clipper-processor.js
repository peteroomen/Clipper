...
class ClipperProcessor extends AudioWorkletProcessor {
  ...
  _createPedal(id, type, params, engaged) {
    ...
    // POSITION/SENSE/VOICE), 'chorus'=the M13.7 CE-1 Chorus Ensemble (the JC-120's
    ...
    // anything else = RAT.
    const t = type === 'sd1' ? 'sd1' : type === 'ts' ? 'ts' : type === 'muff' ? 'muff' : type === 'gold' ? 'gold' : type === 'comp' ? 'comp' : type === 'gate' ? 'gate' : type === 'phaser' ? 'phaser' : type === 'wah' ? 'wah' : type === 'chorus' ? 'chorus' : 'rat';
    const P = t === 'sd1' ? '_sd' : t === 'ts' ? '_ts' : t === 'muff' ? '_muff' : t === 'gold' ? '_gold' : t === 'comp' ? '_comp' : t === 'gate' ? '_gate' : t === 'phaser' ? '_phaser' : t === 'wah' ? '_wah' : t === 'chorus' ? '_chorus' : '_rat';
  ...
  // ('_sd' | '_ts' | '_muff' | '_gold' | '_phaser' | '_wah' | '_chorus' | '_rat').
  _prefix(node) {
    return node.type === 'sd1' ? '_sd' : node.type === 'ts' ? '_ts' : node.type === 'muff' ? '_muff' : node.type === 'gold' ? '_gold' : node.type === 'comp' ? '_comp' : node.type === 'gate' ? '_gate' : node.type === 'phaser' ? '_phaser' : node.type === 'wah' ? '_wah' : node.type === 'chorus' ? '_chorus' : '_rat';
  }

...
- The pedal chain is a LIST you can edit. THREE dirt pedals are available — a RAT-style distortion, an SD-1-style overdrive, and a TS808 'Screamer' (the green box) — and the player can stack MULTIPLE of any, reorder them, add/remove, and the chain may even be EMPTY (guitar straight into the amp). In the rig context each pedal appears in \`pedals\` as an ordered array with its \`type\` ('rat', 'sd1', or 'ts'); address a specific one by its 0-based INDEX (index 0 = first in the chain, closest to the guitar). ORDER MATTERS because distortion is nonlinear: e.g. a Screamer boosting a RAT after it sounds different from the reverse. Use \`add_pedal\` (pass type 'rat', 'sd1', or 'ts'), \`remove_pedal\`, and \`move_pedal\` to shape the board; use the \`pedal\` field on set_param/set_engaged to target an instance (defaults to the first). When there is only one pedal you can ignore the index.

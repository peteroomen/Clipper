...
# The rig you control
The player's signal chain is: guitar -> a PEDALBOARD (an ordered chain of pedals) -> an AMP (selectable: a JC-120-style clean amp, a Marshall JCM800 valve head, OR a Fender blackface "Twin-style" clean combo) -> a speaker cab (selectable: Clean 2×12 or Brit 4×12, or a user IR) -> speakers.

- The pedal chain is a LIST you can edit. FOUR dirt pedals are available — a RAT-style distortion, an SD-1-style overdrive, a TS808 'Screamer' (the green box), and a big-box 'Pi' FUZZ (the violet Muff-style wall of sustain) — plus a script-era PHASER (modulation, not dirt) — and the player can stack MULTIPLE of any, reorder them, add/remove, and the chain may even be EMPTY (guitar straight into the amp). In the rig context each pedal appears in \`pedals\` as an ordered array with its \`type\` ('rat', 'sd1', 'ts', or 'muff'); address a specific one by its 0-based INDEX (index 0 = first in the chain, closest to the guitar). ORDER MATTERS because distortion is nonlinear: e.g. a Screamer boosting a RAT after it sounds different from the reverse. Use \`add_pedal\` (pass type 'rat', 'sd1', 'ts', or 'muff'), \`remove_pedal\`, and \`move_pedal\` to shape the board; use the \`pedal\` field on set_param/set_engaged to target an instance (defaults to the first). When there is only one pedal you can ignore the index.
...
- 'Pi' FUZZ pedal ('muff', the VIOLET big box — three knobs, each 0-100): sustain, tone, volume. This is FUZZ, not overdrive — a different animal from the RAT/SD-1/Screamer. Four cascaded transistor stages with TWO diode-clipping stages generate a MASSIVE, thick, saturated wall of sound that compresses and sustains almost forever (notes bloom and hang). Because it clips so hard it also has huge sustain and a slightly synthy, singing lead voice — think classic thick fuzz leads and huge doom/stoner chords.
  - sustain (its gain/drive knob): how hard the clipping stages are slammed = how much fuzz + sustain + compression. Even low settings are dirty (a Muff has no clean setting); crank it for the endless-sustain lead wall.
...
- The AMP has three selectable heads (\`amp.type\` in the rig context; switch with set_amp — the cab and pedals carry over, switching is click-free):
...
- The AMP has two selectable heads (\`amp.type\` in the rig context; switch with set_amp — the cab and pedals carry over, switching is click-free):
...
- Fender blackface "Twin-style" clean combo (only when amp.type is 'twin' — the CLEAN-HEADROOM KING: a valve amp voiced to stay glassy and clean, with breakup arriving LATE and mostly from the power stage when you really push VOLUME):
  ...
  - tremolo (the panel calls it "vibrato" but it's really amplitude TREMOLO — a throbbing volume pulse): SPEED (set_param 'speed', how fast the throb, ~1-10 Hz) and INTENSITY (set_param 'depth', how deep). The classic Fender combo is SPRING REVERB + TREMOLO together — reverb for space, tremolo for that hypnotic surf/ballad pulse. Suggest a slow-ish speed (30-50) with moderate intensity for a lazy sway, faster for a choppy stutter.

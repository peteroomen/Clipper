...
- **Two tests moved for reasons that look wrong at a glance — read §36 before "fixing" them back.** `testFactorOneRegression`'s hardcoded samples were **regenerated** (they are a drift guard, not a property; the check is that they grew by ~5 dB, the amount the ceiling grew). `testGermaniumKnee`'s bounds were **LOOSENED** (5× → 20× onset, 0.25× → 0.05× slope) — the bug had been making its *genuine* properties harder to satisfy, and the measured margins improved by an order of magnitude at the same time (26× → 189×, 0.032 → 0.0063). Both new absolute tests were perturbation-proven: reverting either ideality factor fails them.

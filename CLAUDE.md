...
- **Just landed:** `fix/proxy-loopback` — the audit's Security & app layer, cleared. The proxy binds `127.0.0.1` by default (`HOST` opts into wider exposure, loudly); the startup banner reports the address actually bound; the Electron main window has a CSP, `will-navigate`/`setWindowOpenHandler` denial and a media-only permission handler; `config.json` is `chmod`ed to `0600` even when it already existed at `0644`; the static-path containment check no longer accepts a sibling-prefix directory; a body-limit overflow destroys the request. Node suites 27 → 35 tests.
- **Audit backlog:** `docs/audits/2026-07-24-project-audit.md` remains the source of truth and supersedes ad-hoc bug lists. Three findings are still shipping-blockers, **none actioned** (NaN parameters permanently brick audio; cab swap runs 11–46 ms of allocation inside `process()`; `CabConvolver` corrupts its stream on non-128-multiple host block sizes).
- **Known gaps (from the audit, not yet actioned):** no CI at all · **the proxy still has no auth and no rate limit** (its own slice; loopback binding is the mitigation until then) · `check-artifact.mjs` only checks existence, not staleness · the valve amps have no parameter smoothing · per-triode oversampling costs 7.5 ms of latency on the JCM800 · the AC30 "sag" is a static saturator and its tone stack has a structural ~37 dB mid notch · a recurring class of test asserts identities or the implementation against a reference derived from the same code.

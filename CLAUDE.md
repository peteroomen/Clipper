...
- **Just landed:** **audit finding 1 is fixed** — the non-finite parameter guard + the engine reset path (`fix/nan-parameter-guard`, docs §28, ADR 002). One shared NaN-rejecting clamp (`core/include/clipper/dsp/ParamGuard.h`) replaces ~14 broken copies, every `*_set_param` C ABI export hard-rejects non-finite, and `reset()` exists down the whole tree with `clipper_reset` / `rat_reset` / `sd_reset` / `ts_reset` / `phaser_reset` / `muff_reset` / `gold_reset` / `amp_reset` exports. Measured 48000/48000 → **0/48000** non-finite samples after one NaN parameter, on all ten units; `Jcm800Amp::reset()` is ~302 000× cheaper than `prepare()`. New ctest target `clipper_nan_guard_tests` (17/17 now). **Note:** the reset exports exist but nothing in the UI calls them yet — a watchdog or an explicit affordance is a follow-up slice.
- **Also just landed: audit finding 2 is fixed** — the cab swap is off the render path (`fix/cab-swap-rt-safety`, stacked on the above; docs §29, ADR 003). The C ABI now **double-buffers the per-side convolver pair** (`CabPair cabs[2]` + an active index, the same pattern `amp_set_model` uses for the amp voices) and splits the one call into `amp_prepare_cab_builtin` / `amp_prepare_cab_custom` (heavy, off the render path, returns 0 on a bad IR) and `amp_commit_cab` (`activeCab ^= 1`). `CabConvolver` is **deliberately untouched** — `fix/cab-block-size` owns that file. Measured: the audio-thread step went from **10.97 ms / 42.66 ms** (411 % / 1600 % of the 2.667 ms deadline) to **~1 ns and zero allocations** (counted with a replaced global `operator new`); worst render block spanning a 4096-tap IR load **104.9 ms → 0.42 ms**, 39 missed quanta → none. Fidelity is **bit-identical** to the pre-fix construction (`max|Δ| == 0.0` over 128 000 samples/side incl. a swap-back). New ctest target `clipper_cab_swap_tests` (18/18 now). Same slice fixed the stale `HEAPF32` across the commit, made a second edit inside one fade window **merge** instead of force-committing at non-zero gain (`_stageEdit`; that path also leaked a WASM handle on `main`), and added a `'hold'` declick phase covering the swapped-in convolver's dead partition. **Note:** `onmessage` runs on the audio *rendering* thread — this moves the work *between* render quanta, it is **not** "now real-time safe"; the main-thread-spectra fix is still ledgered.
- **Still open from the audit:** the 2026-07-24 audit (`docs/audits/2026-07-24-project-audit.md`) remains the source of truth. **One shipping-blocker remains**: `CabConvolver` corrupts its stream on non-128-multiple host block sizes (finding 3).
- **Known gaps (from the audit, not yet actioned):** no CI at all · `check-artifact.mjs` only checks existence, not staleness · the valve amps have no parameter smoothing · per-triode oversampling costs 7.5 ms of latency on the JCM800 · the AC30 "sag" is a static saturator and its tone stack has a structural ~37 dB mid notch · a recurring class of test asserts identities or the implementation against a reference derived from the same code · the SIGNAL path is still NaN-transparent (`OutputLimiter::clamp1`) and `native/src/ClipperEngine` has no reset seam · `_prepareChain` mutates `existing.engaged` on the **live** node, so an engaged-flag change in a `chain` message is not declicked (stomps are, via `_pendingBypass`) · `_destroyPedal`'s `free()` still runs inside `_commitPending`, i.e. inside `process()`.

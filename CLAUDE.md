...
- **Just landed:** a full project audit — `docs/audits/2026-07-24-project-audit.md`. It is the current source of truth for what to fix next and supersedes ad-hoc bug lists. Three findings are shipping-blockers (NaN parameters permanently brick audio; cab swap runs 11–46 ms of allocation inside `process()`; `CabConvolver` corrupts its stream on non-128-multiple host block sizes). Nothing from it has been actioned yet.

...
- **Also just landed (2026-07-25): `fix/web-input-and-a11y`** — the audit's **web UI/UX input + accessibility cluster** (docs §38). **No `core/`, no `web/worklet/`, no `native/`, no artifact rebuild** — `web/src/**` plus one new spec. Seven findings, all measured rather than asserted. **Contrast:** `.pedal` pinned its chassis dark on every theme but left `--accent-*`, `--chassis-tint` **and** `--ai` on the page theme, so **six of seven** pedal accents were under WCAG AA on the light bench — Pi fuzz **2.15:1**, GOLD 2.78:1, phaser 3.32:1, RAT 3.51:1, Screamer 3.99:1, SD-1 4.28:1 → now **4.93 / 6.32 / 7.12 / 8.19 / 8.34 / 10.77:1**, with dark theme unchanged to the digit. **Knobs:** a focus ring at last (2 px `--ai`; ring contrast 2.83 → **6.69:1**), **Shift = fine drag** (1.6 → **8.0 px per 1 %**, measured 5.00×), Shift+arrow 1 %, **PageUp/PageDown ±20 %, Home/End 0/100** — and the drag is now **incremental**, because an anchored drag jumps 0.10 the instant the modifier changes. **Popovers:** one shared `web/src/components/Menu.tsx` gives all three Escape (focus restored to the trigger), pointer-outside dismissal and a real Tab **focus trap**. **"Upload IR…"** is a named `role="menuitem"` button, not a `<label>` wrapping a `display:none` input. **Chat** is a `role="log"` live region and auto-scrolls **only when already pinned** (measured: `scrollTop` 1732 → **0** while reading back). **Tuner rAF while disengaged: 61 → 0 frames/s.** `Board.center()` rejects a zero `DOMRect`, so no cable renders from off the board below 760 px. New `web/tests/a11y.spec.ts` — **15 tests, and 14 of them fail on the pre-fix source** (the one that passes is the dark-theme contrast test, which is the correct outcome).
- **Three things that slice pinned, and one it deliberately left:** custom-property `var()` substitution resolves on the element that **declares** the property, so pinning `--accent-*` on `.pedal` does reach a `--pedal-accent` declared by `.pedal[data-pedal-type=…]` (the same element) but would not reach one declared on an ancestor · the knob focus ring **needs its `border-radius`**, or a square outline slices through the neighbouring knobs' value arcs at the board's 8 px gaps · a `page.mouse` gesture on the board needs `scrollIntoViewIfNeeded` first, or the knob's bounding box is below the fold and the drag silently measures **zero** · `tuner.css:175-180` still **hardcodes two colours** (`#d8863a`/`#3e8fd6`), a genuine no-hardcoded-colour violation that is not in the audit, left alone because naming tokens for them is a palette decision, not a defect fix.
- **Observed on `main`, not caused by any slice:** the Playwright suite is **red on `audio.spec.ts` `muff worklet` and `gold worklet`** (and `TS worklet` intermittently) — the Muff renders RMS **exactly 0** for the −30 dBFS leg of its dynamics test, so `hotRms/softRms` is `Infinity`. Reproduced on a clean tree *and* against the **pre-#12** committed `clipper.js`, so it is neither the a11y slice nor the Muff Newton early-out. Needs an owner.

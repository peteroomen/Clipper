...
- **Still open from the audit:** `docs/audits/2026-07-24-project-audit.md` remains the source of truth. **Finding 15 (the diode ideality factor) is the first circuit defect fixed** — see below. The rest of Phases 2–5 is untouched: the AC30 sag + tone-stack topology, the missing amp parameter smoothing, the Muff's absent output DC blocker and bass loss, and the shared PI tail reference.
...
| Command                                        | Purpose                                                       |
| ---------------------------------------------- | ------------------------------------------------------------- |
| `ctest --test-dir build --output-on-failure`   | The core DSP suite (19 targets + 6 `_xfail_ledger` entries reported Skipped) |

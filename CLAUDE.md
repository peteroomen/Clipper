...
- **Still open from the audit:** `docs/audits/2026-07-24-project-audit.md` remains the source of truth. **Three circuit defects are now fixed: finding 15** (the diode ideality factor, docs §36), **finding 6** (valve-amp parameter smoothing, docs §35) **and finding 7** (the PI tail reference, docs §42). Remaining physics cluster — findings 4, 5, 8, 9, 10: the AC30 sag and its ~37 dB mid notch, `Ra2` (re-measure the sweep post-tailRef; the JCM ratio is 0.703 and its even-harmonic cancellation 8.1 dB), the single-ended plate load line, the screen-current fits. **Finding 16 is CLOSED** (docs §37 the output DC blocker, §49 the series base resistors, §53/ADR 010 the DC-blocking diode caps that ADR 009 correctly predicted were the upstream error).
...
| Command                                        | Purpose                                                       |
| ---------------------------------------------- | ------------------------------------------------------------- |
| `ctest --test-dir build --output-on-failure`   | The core DSP suite (21 targets + 3 `_xfail_ledger` entries reported Skipped — 24 ctest entries) |
| `ctest --test-dir build --output-on-failure`   | The core DSP suite (21 targets + 4 `_xfail_ledger` entries reported Skipped — 25 ctest entries; the GOLD ledger closed with docs §54) |

...
- **Still open from the audit:** `docs/audits/2026-07-24-project-audit.md` remains the source of truth. **Finding 6 is fixed by this slice** (see below). The valve-amp *physics* cluster is untouched: findings 4, 5, 7, 8, 9, 10 — the AC30 sag and its ~37 dB mid notch, the shared PI tail reference, `Ra2`, the single-ended plate load line, the screen-current fits. Those share files and calibration constants, so they are sequential by necessity and are the largest remaining chunk of the "doesn't sound like the real amp" surface.

...
- **Design tokens** live in `web/src/styles/tokens.css`. Never hardcode a colour — it must work in both themes or come from a token. `.pedal` pins its chassis to the dark tokens on **every** theme, so **anything painted on that chassis must read the theme-invariant `*-on-dark` palette**, not the page theme: the accents, `--led*`, `--ai`, `--chassis-tint` and the tuner `--seg-*` group are pinned in one block in `pedal.css` (docs §38.1). Add a new pedal accent to the on-dark set *and* to that block, or it will look right in dark theme and fail WCAG AA on the light bench. `web/tests/a11y.spec.ts` computes the real ratio per pedal per theme and will tell you.
- **Knobs** are the most numerous control in the app: vertical drag with pointer capture (**incremental**, so holding **Shift** for a 5× fine drag never makes the value jump), mouse wheel, double-click to default, a visible focus ring, the full ARIA slider keyboard contract (arrows 5 %, Shift+arrow 1 %, PageUp/PageDown 20 %, Home/End to the ends) and `role="slider"` with live `aria-valuenow`/`aria-valuetext`.
- **Every popover goes through `web/src/components/Menu.tsx`.** Don't hand-roll `{open && <div role="menu">}` again — that shipped three menus with no Escape, no click-outside and no focus trap. The shared component owns all of it plus `aria-haspopup`/`aria-expanded` and focus restoration to the trigger.

...
- **Still open from the audit:** `docs/audits/2026-07-24-project-audit.md` remains the source of truth. **Two circuit defects are now fixed: finding 15** (the diode ideality factor, docs §36) **and finding 6** (valve-amp parameter smoothing, docs §35, this slice). The valve-amp *physics* cluster is untouched — findings 4, 5, 7, 8, 9, 10: the AC30 sag and its ~37 dB mid notch, the shared PI tail reference, `Ra2`, the single-ended plate load line, the screen-current fits. Those share files and calibration constants, so they are sequential by necessity and are the largest remaining chunk of the "doesn't sound like the real amp" surface. Finding 16 is half done (the Muff's output DC blocker; its bass half is deferred with a named reason — ADR 009).
...
- **Also just landed (2026-07-25): `test/web-render-silence-guard`** — the docs §29 residual closed, and **`retries: 2` is gone** (docs §41, ADR 013). Tests + config + docs only, **zero application source change**, no WASM rebuild, and not one audio bound loosened. The intermittent all-zero Playwright render was **not** `OfflineAudioContext` accumulation (a no-worklet context rendered 16/16; the failing path goes silent on the *first* context of a fresh page; and `OfflineAudioContext.prototype.close` does not exist in this Chromium, so "close the contexts" was never callable) — it is a **race between the `AudioWorkletProcessor` being installed onto the offline render thread and `startRendering()`**, because the `ready`/`latency` messages the specs await are posted from `AudioWorkletGlobalScope`, a *different* thread from the one that installs the processor into the render graph. Fixed with a real happens-before barrier (suspend at frame 0 → `startRendering()` → await → resume), **bit-transparent** at 0 differing samples / 24 000 over 5 trials, now installed in **all five** audio-rendering specs via one `beforeEach`. Measured under a fixed 6-way CPU load on 4 cores, 5 full-suite runs each at `--retries=0`: **41 failures / 355 test-executions, 5/5 runs red → 0 / 355, 5/5 green** (15 distinct tests were affected; the worst failed 5/5). A silent render now **names itself** instead of surfacing as `toBeGreaterThan(0.01)` / `Received: 0`, proven by perturbation (delete `osc.start()`: guard on → `SILENT RENDER … SETUP FAILED`; guard off → `Received: 0`). The three `setTimeout(() => resolve(), 6000)` setup waits now **reject**. And the old comment's *"a genuine failure loses all attempts"* is refuted with a 20 %-per-render injected fault over 25 repetitions: `--retries=0` → 5 failed, exit 1; **`--retries=2` → 0 failed, 6 flaky, exit 0 (suite green)**.
- **Two things this slice found and did NOT fix:** `amp twin: the optical tremolo modulates the output envelope` has a *second*, different fault — one baseline run gave `onCV` **exactly equal** to `offCV` (0.019677767321650944 vs `offCV*4` = 0.07871106928660378), i.e. the INTENSITY 0.0 and 0.8 renders were **bit-identical**, so the param/ampModel messages never reached the render. That is not silence (silence gives `envCV` 0) and it is the same shape as §29's `voiceDiff`-exactly-0 case; it did not recur in 5 clean post-fix runs, but if it comes back, **measure it, do not retry it**. And **`web/tsconfig.json` excludes `tests`**, so `npm run build`'s `tsc --noEmit` typechecks **no spec at all** and Playwright's esbuild transpile does not either — a type error in a spec is invisible to the entire gate (hand-checked clean this slice; `cab.spec.ts` needs `@types/node`, which is not a dependency).
...
>
> **What CI cannot catch, and you must therefore still think about:** the goldens are `.wav` files no reviewer can read in a diff (the Goldens-changelog job forces a written justification, but cannot check it is *true*). See docs §29. **`retries` is now `0`** — a flake is a failure, so do not "fix" a red Playwright job by raising it; the mechanism behind the old intermittent all-zero render is fixed and documented in docs §41, and the config comment carries the measured numbers.

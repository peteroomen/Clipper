...
- **Still open from the audit:** `docs/audits/2026-07-24-project-audit.md` remains the source of truth and supersedes ad-hoc bug lists. **One shipping-blocker remains**: cab swap runs 11–46 ms of allocation inside `process()` (finding 2) — in flight as `fix/cab-swap-rt-safety`. Finding 3 (`CabConvolver` corrupting its stream on non-128-multiple host block sizes) is fixed and awaiting merge as `fix/cab-block-size`.

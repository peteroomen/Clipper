...
- **All three audit shipping-blockers are fixed** (2026-07-25). In merge order:
  - **CI exists at last** (`.github/workflows/ci.yml`) — core ctest, web tsc+vite+Playwright, node suites, a dependency-free Conventional Commits gate, JUCE identical-core (advisory).
  - **Security & app layer cleared** (`fix/proxy-loopback`) — the proxy binds `127.0.0.1` by default (`HOST` opts into wider exposure, loudly) and the banner reports the address actually bound; the Electron window has a CSP (including `'wasm-unsafe-eval'` — without it WASM never compiles and the app makes no sound), `will-navigate`/`setWindowOpenHandler` denial, and a media-only permission handler; `config.json` is `chmod`ed `0600` even when it pre-existed at `0644`. Node suites 27 → 35.
  - **Finding 1 — NaN parameters** (docs §28, ADR 002): one shared NaN-rejecting clamp (`core/include/clipper/dsp/ParamGuard.h`) replaces ~14 broken copies, every `*_set_param` export hard-rejects non-finite, and `reset()` exists down the whole tree (8 exports) re-parking from a cached DC point — `Jcm800Amp` 87.59 ms → 0.0003 ms. Measured 48000/48000 → **0/48000** non-finite after one NaN, all ten units. `Inf` is now rejected rather than clamped.
  - **Finding 3 — CabConvolver block sizes**: was misaligning its stream forever on any non-128-multiple block (at 100 samples the error EXCEEDED the signal). Now exact at every size 1–4096, at **no** added latency, 128-aligned path bit-identical.
  - **Finding 2 — cab swap on the render path** (docs §29, ADR 003): split into `amp_prepare_cab_*` + `amp_commit_cab`, double-buffered at the ABI. The audio-thread step is **0.000001 ms and 0 allocations** against a prepare of 10.97–42.66 ms. Worst render block over a 4096-tap IR load: 104.92 ms (39 quanta missed) → **0.42 ms (none)**. Bit-identical to the pre-fix audio.
- **Honest residual on finding 2:** `amp_prepare_cab_*` runs in the worklet's `onmessage`, which **is** the audio rendering thread. This moves the work *between* quanta rather than off-thread — a guaranteed multi-quantum stall becomes at worst one late quantum on a deliberate user action. It is NOT "real-time safe". The real fix is main-thread spectra. Also still on the audio thread: `_destroyPedal`'s `free()` inside `_commitPending`, and `_postLatency()` posting from `process()`.
- **Still open from the audit:** `docs/audits/2026-07-24-project-audit.md` remains the source of truth and supersedes ad-hoc bug lists. Nothing from Phases 2–5 has been actioned.
- **Known gaps:** the proxy has no auth and no rate limit (loopback is the mitigation) · `check-artifact.mjs` only checks existence, not staleness · the valve amps have no parameter smoothing · per-triode oversampling costs 7.5 ms of latency on the JCM800 · the AC30 "sag" is a static saturator and its tone stack has a structural ~37 dB mid notch · a recurring class of test asserts identities or the implementation against a reference derived from the same code · the SIGNAL path is still NaN-transparent (`OutputLimiter::clamp1`) · `native/src/ClipperEngine` has no reset seam and still only chunks when `numFrames > maxBlock_`.
- **Found while fixing, not yet fixed:** `_prepareChain` mutates `engaged` on the *live* node, so an engaged-flag change arriving in a `chain` message is not declicked (a stomp via `_pendingBypass` is). `CabConvolver::prepare()` zeroing the FDL means a swapped-in cab is silent for up to two partitions — handled with a 256-sample `'hold'` declick phase rather than by touching the convolver.
...
- **ADR numbers are assigned centrally.** Two parallel slices both reached for `001`. Ask rather than guess.
- **CI native job:** it had failed on every run with `continue-on-error` hiding it — `ctest` in `native/build` inherits the core's registered targets and reports them Not Run. Fixed with `-R`. It stays advisory until observed green: the load-bearing identical-core test had **never** actually run in CI.
...
| Command                                        | Purpose                                                       |
| ---------------------------------------------- | ------------------------------------------------------------- |
| `ctest --test-dir build --output-on-failure`   | The core DSP suite (18 targets)                               |

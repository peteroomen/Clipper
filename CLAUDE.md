...
- **Still open from the audit:** the 2026-07-24 audit (`docs/audits/2026-07-24-project-audit.md`) remains the source of truth. **Two shipping-blockers remain**: cab swap runs 11–46 ms of allocation inside `process()` (finding 2); `CabConvolver` corrupts its stream on non-128-multiple host block sizes (finding 3).
...
- **The 2026-07-24 audit backlog** — `docs/audits/2026-07-24-project-audit.md` is the ordered list. Its own suggested order: the three shipping-blockers (finding 1 done 2026-07-25; findings 2 and 3, both `CabConvolver`-adjacent, remain), then bind the proxy to loopback, then stand up CI *before* the circuit work (so fidelity changes are measured rather than asserted), then the AC30 sag + stack topology + the shared PI tail reference.

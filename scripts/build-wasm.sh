...
EMCC_ARGS=(
    ...
    "$CORE_DIR/src/dsp/MesaPowerAmp.cpp"
    "$CORE_DIR/src/dsp/MesaAmp.cpp"
    # M13.10 drop-tune (docs §70) — inside the STAMP:EMCC-ARGS markers.
    "$CORE_DIR/src/dsp/DropModel.cpp"
    # M13.6 ten-band graphic EQ (docs §71) — inside the STAMP:EMCC-ARGS markers.
    # This list is EXPLICIT: §60, §64, §69 and §70 each record a slice whose
    # first artifact rebuild failed at wasm-ld with undefined symbols because a
    # new .cpp was not added here. The C++ compiles clean either way.
    "$CORE_DIR/src/dsp/EqModel.cpp"
    -I "$CORE_DIR/include"
    -isystem "$WDF_INC"
    -O3
    -msimd128
